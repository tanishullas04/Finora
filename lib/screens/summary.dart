...
class _SummaryScreenState extends State<SummaryScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      // Parse income
      double income =
          ((incomeData?['salary'] ?? 0.0) as num? ?? 0.0).toDouble() +
      ...
      // Parse deductions
      double deductions =
          ((deductionsData?['section80c'] ?? 0.0) as num? ?? 0.0).toDouble() +
      ...
      // Parse capital gains
      double stcgTotal = ((cgData?['totalSTCG'] ?? 0.0) as num? ?? 0.0)
          .toDouble();
      double ltcgRealEstate =
          ((cgData?['ltcgRealEstate'] ?? 0.0) as num? ?? 0.0).toDouble();
      double ltcgStocks = ((cgData?['ltcgStocks'] ?? 0.0) as num? ?? 0.0)
          .toDouble();
      double ltcgMutualFunds =
          ((cgData?['ltcgMutualFunds'] ?? 0.0) as num? ?? 0.0).toDouble();
      double ltcgOther = ((cgData?['ltcgOther'] ?? 0.0) as num? ?? 0.0)
          .toDouble();

      // Parse GST
      double gstTax = ((gstData?['totalGSTTax'] ?? 0.0) as num? ?? 0.0)
          .toDouble();
      ...
      // Calculate LTCG tax
      double ltcgTax = _calculateLTCGTax(
        ltcgRealEstate,
        ltcgStocks,
        ltcgMutualFunds,
        ltcgOther,
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            ListTile(
                              title: const Text('Salary & Other Income'),
                              trailing: Text(_formatCurrency(_totalIncome)),
                            ),
                            ListTile(
                              title: const Text('STCG (Short-term Gains)'),
                              trailing: Text(_formatCurrency(_stcgTotal)),
                            ),
                            ListTile(
                              title: const Text('Total Income'),
                              trailing: Text(
                                _formatCurrency(_totalIncome + _stcgTotal),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            ListTile(
                              title: const Text('Deductions (80C/80D/24)'),
                              trailing: Text(_formatCurrency(_totalDeductions)),
                            ),
                            ListTile(
                              title: const Text('LTCG Tax'),
                              trailing: Text(_formatCurrency(_ltcgTax)),
                            ),
                            ListTile(
